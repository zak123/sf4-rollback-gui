...
This page preserves upstream sf4e's build and run documentation, with
notes for this fork's additional targets at the end.
...
## This fork's additional targets

A full build also produces:

* `LobbyClient.exe` — the desktop lobby app (chat, lobby browser,
  launch-into-match).
* `Lobbyd.exe` — the dedicated, headless lobby server. See
  [vps-playtest.md](vps-playtest.md) for hosting it.
* `SessionSmokeTest.exe` — a headless test that drives a real server and
  clients through the whole session protocol over loopback. **Run it
  after touching anything in `src/session/`**; CI runs it as a merge
  gate.
* `SessionInteractiveTest.exe` — upstream's interactive protocol
  harness.

`scripts/local-test.bat` spins up a local server plus two lobby apps for
one-machine testing (`scripts/local-test.bat stop` tears them down).
CI (`.github/workflows/build.yml`) builds all of this on every push and
packages the client/server bundles.
